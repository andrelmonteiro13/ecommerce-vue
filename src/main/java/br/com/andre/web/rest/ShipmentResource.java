...
/**
 * REST controller for managing {@link br.com.andre.domain.Shipment}.
 */
@RestController
...
public class ShipmentResource {
    ...
    public Mono<ResponseEntity<List<ShipmentDTO>>> getAllShipments(
        ...
        return shipmentService
            ...
            .zipWith(shipmentService.findByCriteria(criteria, pageable).collectList())
     ...
     */
    @GetMapping("/count")
    public Mono<ResponseEntity<Long>> countShipments(ShipmentCriteria criteria) {
        LOG.debug("REST request to count Shipments by criteria: {}", criteria);
        return shipmentService.countByCriteria(criteria).map(count -> ResponseEntity.status(HttpStatus.OK).body(count));
    }
